(function(){
  ...
  function enhanceReel(){
    try{
      ...
      var pointer = { down:false, startX:0, lastX:0, startTime:0, isDragging:false };
      reel.addEventListener('pointerdown', function(e){
        pointer.down = true; pointer.startX = e.clientX; pointer.lastX = e.clientX; pointer.startTime = Date.now(); pointer.isDragging = false;
        try{ reel.setPointerCapture && reel.setPointerCapture(e.pointerId); }catch(err){}
      }, { passive:false });
      reel.addEventListener('pointermove', function(e){
        if(pointer.down){
          pointer.lastX = e.clientX;
          // if user moved more than a few pixels, consider it a drag
          if(Math.abs(pointer.lastX - pointer.startX) > 6) pointer.isDragging = true;
        }
      }, { passive:true });
      reel.addEventListener('pointerup', function(e){
        ...
        // clear dragging flag shortly after pointer up to avoid blocking clicks when appropriate
        setTimeout(function(){ pointer.isDragging = false; }, 50);
      });

      // Suppress click events triggered by end of drag so lightbox/anchor clicks don't fire
      reel.addEventListener('click', function(e){
